...
jQuery( document ).ready( function($){
	...
	*/
	...
	var fakeParallaxContainer = '.fake-parallax';
	...
	var templateFunctions = {
		init: function(){
			...
			templateFunctions.fakeParallax( '.fake-parallax.fixed-height', true, false, true );
			templateFunctions.fakeParallax( '.fake-parallax.fullscreen', true, true, true );
			// Fullscreen sections
			...
			// Google Map
			if( $( '.map-canvas' ).length ) google.maps.event.addDomListener( window, 'load', templateFunctions.googleMap );
			// Page Fade
		...
		fakeParallax: function( container, scale, fullscreen, fade ){
			$( container ).snowBridge({
				scaleContainer: scale,
				scaleUnder: 960,
				scaleMinHeight: 214,
		...
		teamThreeSlider: function(){
			$( teamThreeSlider ).each( function() {

				var $this = $( this ),
					columns = parseInt( $this.attr( 'data-columns' ) );

				$this.avalancheSlider({
					animation: 'slide',
					easing: 'easeInOutQuart',
					speed: 900,
					autoAdvance: false,
					carousel: true,
					carouselVisible: columns,
		...
		googleMap: function(){
			...
			$maps.each( function() {
				...
				var $this = $( this ),
					$section = $this.closest( '.section-type-google-map' ),
					$locations = $section.find( '.section-type-google-map-location' );

				mapOptions.zoom = parseInt( $this.attr( 'data-zoom' ), 10 ) || 0;
				...
				if ( $locations.length ) {
					...
					$locations.each( function() {
						...
						var $this = $( this ),
							...
							markerWidth = $this.attr( 'data-marker-width' ),
						...
						latlng.push( markerObj.position );

						if ( marker ) {

							markerObj.icon = {
								url       : marker,
								scaledSize: new google.maps.Size( parseInt( markerWidth, 10 ), parseInt( markerHeight, 10 ) )
							};

						}

						var locationMarker = new google.maps.Marker( markerObj );
